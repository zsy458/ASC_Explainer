...
class Bert_grus(nn.Module):
    ...
    def forward(self, inputs, masks, segment_mask, device):
        masks_ = masks.unsqueeze(-1)
        gen_bert_out=self.gen_encoder(inputs, attention_mask=masks, token_type_ids=segment_mask)[0]
        # enc = self.dropout(gen_bert_out)
        # gen_logits,_ = self.gen(enc)
        # output_logits = gen_logits.mean(dim=-1)
        output_logits = self.gen_linear(gen_bert_out)     
    ...
    '''
    def get_rationale(self, inputs, masks, special_masks, device):
        masks_ = masks.unsqueeze(-1)
        gen_bert_out=self.gen_encoder(inputs,masks)[0]
        # print('gen_bert_out')
        # print(gen_bert_out.size())
        ...
        # print('gen_logits.size:',gen_logits.size())
        # print('gen_logits.:',gen_logits)

        # output_logits = gen_logits.mean(dim=-1)
        output_logits = self.gen_linear(gen_logits)     

        # print('output_logits.size:',output_logits.size())
        # print('output_logits:',output_logits)
        rationale = self.independent_straight_through_sampling(output_logits) # (batch_size, seq_length, 2)
        # print('rationale:',rationale)
        # print('rationale:',torch.count_nonzero(rationale).item())
        # print('rationale')
        unrationale = 1 - rationale
        # print('unrationale:',torch.count_nonzero(unrationale).item())
        # a = hh

        # print(rationale.size())
        # print('special_masks')
        # print(special_masks.size())

        # z_add_special_tokens=torch.max(rationale[1,:,:],special_masks)     #cls和sep恒为1z
        z_add_special_tokens_ran=torch.max(rationale[:,:,1],special_masks)     #cls和sep恒为1
        z_add_special_tokens=torch.max(unrationale[:,:,1],special_masks)     #cls和sep恒为1



        # return rationale[1,:,:], z_add_special_tokens
        return rationale[:,:,1], unrationale[:,:,1]
    '''
