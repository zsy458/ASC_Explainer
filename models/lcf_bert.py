...
class LCF_BERT(nn.Module):
    ...
    def forward(self, inputs,rational_mask):
        ...
        res = self.bert_spc(text_bert_indices, token_type_ids=bert_segments_ids, rationale_mask = rational_mask )
        # res = self.bert_spc(text_bert_indices, attention_mask=rational_mask,token_type_ids=bert_segments_ids)
        bert_spc_out = res.last_hidden_state
        bert_spc_out = self.dropout(bert_spc_out)

        res = self.bert_local(text_bert_indices, token_type_ids=bert_segments_ids, rationale_mask = rational_mask )
        # res = self.bert_local(text_local_indices,attention_mask=rational_mask)
        bert_local_out = res.last_hidden_state
