...
class TD_LSTM(nn.Module):
    ...
    def forward(self, inputs, rational_mask):
        ...
        ration_tmp = torch.roll(rational_mask, shifts=-1).long()
        x_l_tmp = torch.mul(x_l, ration_tmp)
        x_r_tmp = torch.mul(x_r, ration_tmp)
        x_l_len, x_r_len = torch.sum(x_l != 0, dim=-1), torch.sum(x_r != 0, dim=-1)
        x_l, x_r = self.embed(x_l_tmp), self.embed(x_r_tmp)
