...
from data_utils import build_tokenizer, Tokenizer4BertGCN, ABSAGCNData

from data_utils import build_tokenizer, build_embedding_matrix, Tokenizer4Bert, ABSADataset,pad_and_truncate
...
class Instructor:
    def __init__(self, opt):
        ...
        if 'bert' in opt.model_name:
            ...
            self.tokenizer1 = build_tokenizer(
                fnames=[opt.dataset_file['train'], opt.dataset_file['test']],
                max_seq_len=opt.max_seq_len,
                dat_fname='{0}_tokenizer.dat'.format(opt.dataset))
            embedding_matrix = build_embedding_matrix(
                word2idx=self.tokenizer1.word2idx,
                embed_dim=opt.embed_dim,
                dat_fname='{0}_{1}_embedding_matrix.dat'.format(str(opt.embed_dim), opt.dataset))
            self.model = opt.model_class(embedding_matrix, opt).to(opt.device)
        # tokenizer1 = Tokenizer4Bert(opt.max_seq_len, opt.pretrained_bert_name)
        if 'syntactic' in opt.dataset:
            tokenizer = Tokenizer4BertGCN(opt.max_length, opt.pretrained_bert_name)
            self.trainset = ABSAGCNData(opt.dataset_file['train'], tokenizer, opt=opt)
            self.testset = ABSAGCNData(opt.dataset_file['test'], tokenizer, opt=opt)
        self.trainset = ABSADataset(opt.dataset_file['train'], self.tokenizer,self.tokenizer1)
        self.testset = ABSADataset(opt.dataset_file['test'], self.tokenizer,self.tokenizer1)
    ...
    def _train(self, criterion, optimizer, train_data_loader, val_data_loader):
        ...
        for i_epoch in range(self.opt.num_epoch):
            ...
            for i_batch, batch in enumerate(train_data_loader):
                ...
                attention_mask = batch['attention_mask'].to(self.opt.device).float()
                sp_mask = batch['special_mask'].to(self.opt.device).float()
                full_mask = torch.max(attention_mask,sp_mask)
                outputs = self.model(inputs,full_mask)
    ...
    def _evaluate_acc_f1(self, data_loader):
        ...
        with torch.no_grad():
            for i_batch, t_batch in enumerate(data_loader):
                ...
                attention_mask = t_batch['attention_mask'].to(self.opt.device).float()
                sp_mask = t_batch['special_mask'].to(self.opt.device).float()
                full_mask = torch.max(attention_mask,sp_mask)
                t_targets = t_batch['polarity'].to(self.opt.device)
                t_outputs = self.model(t_inputs,full_mask)
    ...
    def test(self, textlist):
        ...
        for text in textlist:
            ...
            full_indicts = self.tokenizer.text_to_sequence('[CLS] ' + text +' [SEP] ' + aspect + " [SEP]")
            ...
            full_indicts =[full_indicts]
            input = [full_indicts,concat_segments_indices]
            input_ = torch.tensor(input).to(self.opt.device)
            # for i in range(0,len(input_)):
                # input_[i]= input_[i].unsqueeze(0)
            attention_mask = torch.tensor(attention_mask).to(self.opt.device)
            single_res = self.model(input_,attention_mask)
...
def main():
    ...
    parser.add_argument('--num_epoch', default=500, type=int, help='try larger number for non-BERT models')
    ...
    input_colses = {
        'td_lstm': ['left_with_aspect_indices', 'right_with_aspect_indices'],#aaaaa
        'atae_lstm': ['text_indices', 'aspect_indices'],#aaaaa
        'bert_spc': ['concat_bert_indices', 'concat_segments_indices','attention_mask'],#aaaaa
        'lcf_bert': ['concat_bert_indices', 'concat_segments_indices', 'text_bert_indices', 'aspect_bert_indices'],#aaaaa
